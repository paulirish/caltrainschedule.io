...
(function () {
    ...
    function second2str(seconds) {
        ...
        if (opts.amPM) {
            ...
            hoursDesc = ((hours + 11) % 12 + 1) + '';
        }
        else
            hoursDesc = (hours % 24).toString().rjust(2, '0');
        var minutesDesc = (minutes % 60).toString().rjust(2, '0');
        return [hoursDesc, ':', minutesDesc, suffix.small()].join('');
    }
    ...
    function get_service_ids(calendar, calendar_dates) {
        ...
        if (target_schedule === 'now') {
            ...
            switch ((new Date().getDay() + 6) % 7) {
                case 5:
                    target_schedule = 'saturday';
                    break;
                case 6:
                    target_schedule = 'sunday';
                    break;
                case 0:
                case 1:
                case 2:
                case 3:
                case 4:
                    target_schedule = 'weekday';
                    break;
                default:
                    console.error('Unknown current day', (new Date().getDay() + 6) % 7);
                    return [];
            }
        }
        // calendar:
    ...
    function schedule() {
        var stops = data.stops, routes = data.routes, bombardiers = data.bombardiers, calendar = data.calendar, calendar_dates = data.calendar_dates;
        var from_ids = stops[$('#from select').value], to_ids = stops[$('#to select').value], services = get_available_services(routes, calendar, calendar_dates);
